...
# ============================================================
# AMBIL TURNSTILE TOKEN (via Node.js Playwright)
# ============================================================
def get_turnstile_token():
    result = subprocess.run(
        ['node', 'get_turnstile.js'],
    ...
    for line in result.stdout.splitlines():
        if line.startswith('RESULT_JSON:'):
            ...
            if data.get('success'):
                return data['turnstileToken'], data['elapsedMs']
            raise Exception(f'Gagal ambil turnstile token: {data.get("error")}')
    raise Exception(f'Gak ketemu RESULT_JSON di output node: {result.stdout}\n{result.stderr}')
...
def login(privkey):
    ...
    turnstile_token, elapsed_ms = get_turnstile_token()

    r = requests.post(f'{BASE_URL}/users/nonce', json={
        'address': address,
        'turnstileToken': turnstile_token,
        'referrerInviteCode': None,
        'hp': '',
        'elapsedMs': elapsed_ms,
        'interacted': True,
    }, headers=api_headers())
    print(f'[DEBUG] nonce status={r.status_code} body={r.text}')
    nonce_data = r.json()
    if r.status_code != 200 or 'message' not in nonce_data:
        raise Exception(f'Nonce gagal: {nonce_data}')
    ...
    r = requests.post(f'{BASE_URL}/users/login', headers=api_headers(), json=payload)
    print(f'[DEBUG] login payload={payload}')
    print(f'[DEBUG] login status={r.status_code} body={r.text}')
    data = r.json()

    # Fallback: kalau server 500 pas ngirim referrerInviteCode (biasanya karena akun
    # udah pernah register / udah punya referrer), coba lagi tanpa field itu.
    if 'accessToken' not in data and r.status_code == 500:
        payload.pop('referrerInviteCode', None)
        r = requests.post(f'{BASE_URL}/users/login', headers=api_headers(), json=payload)
        data = r.json()
